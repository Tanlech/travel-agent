...
from app.agent.agents.prompt.reflection import REFLECTION_AGENT_PROMPT
from app.agent.agents.schema.reflection import ReflectionIssue, ReflectionLLMResult, ReflectionResult
from app.agent.agents.sparse.reflection import build_reflection_user_prompt
from app.agent.domain.common.itinerary import ItineraryDayPlan
from app.agent.domain.context.planning import PlanningContext
from app.infrastructure.llm_client import get_llm_client


def _to_minutes(hhmm: str) -> int | None:
    """'HH:MM' → 当日分钟数；解析失败返回 None"""
    try:
...
class PlanningReflection:
    ...
    def _check_transport_reasonableness(self, state: PlanningContext, days: set[int] | None = None) -> list[ReflectionIssue]:
        """相邻景点跨区移动的交通衔接合理性：总间隔过短（<30min）视为交通时长不足风险，
        提示补充真实交通时长或调整节奏（配合改稿注入的真实交通证据闭环，避免强凑跨区动线）"""
        issues: list[ReflectionIssue] = []
        if not state.draft:
            return issues
        for day in self._iter_days(state, days):
            attraction_indices = [i for i, block in enumerate(day.time_blocks) if block.item_type == "attraction"]
            for left_idx, right_idx in zip(attraction_indices, attraction_indices[1:]):
                current, following = day.time_blocks[left_idx], day.time_blocks[right_idx]
                if not current.area or not following.area or current.area == following.area:
                    continue
                gap = self._gap_minutes(current.end_time, following.start_time)
                if gap is not None and gap < 30:
                    issues.append(
                        ReflectionIssue(
                            code="transport_gap_too_short",
                            message=f"Day{day.day_index} {current.title} → {following.title} 跨区域移动间隔仅 {gap} 分钟，交通时长可能不足",
                            severity="warning",
                            scope="daily_plan",
                            fix_hint=f"action=adjust_time; day_index={day.day_index}; 为跨区相邻景点补充/延长交通时长（≥30分钟）。",
                        )
                    )
        return issues

    @staticmethod
    def _gap_minutes(end_time: str, start_time: str) -> int | None:
        """相邻块间隔分钟数（下一块 start - 上一块 end）；时间格式 'HH:MM'，解析失败返回 None"""
        end, start = _to_minutes(end_time), _to_minutes(start_time)
