...
from app.agent.agents.prompt.revise import (
    REVISE_BLOCK_PROMPT,
    REVISE_DAY_PROMPT,
...
from app.agent.agents.schema.revise import (
...
from app.agent.agents.sparse.revise import build_block_level_revise_prompt, build_day_level_revise_prompt, build_global_revise_prompt, build_revision_intent_prompt
...
from app.agent.tools.schema.transport import TransportInput, TransportResult
...
class ReviseAgent:
    ...
    def run(self, agent_input: ReviseAgentInput) -> ReviseAgentOutput:
        ...
        return self._review_and_repair(
            ...
            transport_evidence=refreshed_context.get("transport"),
        )
    ...
    def _block_level_revise(
        ...
        new_plan = self._render_trip_plan_from_draft(new_draft, agent_input, transport_evidence=refreshed_context.get("transport"))
    ...
    def _global_revise(
        ...
        new_plan = self._render_trip_plan_from_draft(revised_draft, agent_input, transport_evidence=refreshed_context.get("transport"))
    ...
    def _render_trip_plan_from_draft(
        self, draft: ItineraryDraftSchema, agent_input: ReviseAgentInput,
        transport_evidence: dict[tuple[str, str], TransportResult] | None = None,
    ):
        daily_plan = []
        ...
        evidence = transport_evidence or {}
        transport_plan = []
        for day in draft.day_plans:
            transitions = []
            ...
            for idx in range(len(blocks) - 1):
                ...
                # 注入 transport tool 真实证据（总结级：模式/时长/距离/费用），无证据时保留默认
                route = evidence.get((current_block.title, next_block.title))
                if route is not None:
                    mode_summary = pick_transport_mode(route)
                    if mode_summary:
    ...
    def _review_and_repair(
        self, *, agent_output: ReviseAgentOutput, agent_input: ReviseAgentInput,
        transport_evidence: dict[tuple[str, str], TransportResult] | None = None,
    ) -> ReviseAgentOutput:
        from app.agent.domain.context.planning import PlanningContext
        ...
        if state.reflection_result.status == "revise":
            ...
            state.plan = self._render_trip_plan_from_draft(state.draft, agent_input, transport_evidence=transport_evidence)
    ...
    def _refresh_transport(self, agent_input: ReviseAgentInput, impact: RevisionImpactAnalysis) -> dict[tuple[str, str], TransportResult]:
        """按受影响天刷新交通证据：覆盖全部相邻景点对（上限 12 段），返回 (from,to) → TransportResult 映射。

        使用 transport tool 的总结级结果（真实时长/距离，不引入 AMap 原始 step 详情）；
        结果由 _render_trip_plan_from_draft 注入 transport_plan 作为可解释的交通证据"""
        current_draft = agent_input.current_draft or self._recover_draft(agent_input)
        target_days = set(impact.affected_days)
        evidence: dict[tuple[str, str], TransportResult] = {}
        for day in current_draft.day_plans:
            ...
            for idx in range(len(attraction_titles) - 1):
                from_name, to_name = attraction_titles[idx], attraction_titles[idx + 1]
                if not from_name or not to_name or from_name == to_name:
                    continue
                routes = transport_tool.run(
                    TransportInput(
                        city=agent_input.request.destination,
                        from_name=from_name,
                        to_name=to_name,
                ...
                if first is not None and not first.error:
                    evidence[(from_name, to_name)] = first
                if len(evidence) >= 12:
                    return evidence
        return evidence
