...
from app.agent.domain.intent.schema import IntentRecognitionInput
...
四、意图判断（优先级从高到低，请严格按此顺序判断，靠语义与原则，不要依赖某个具体说法）
根据语义判断属于：new_plan / revise_plan / clarification / qa / confirm / reject / end_session / unknown。
- 闲聊优先：凡原话不含明确出行规划信号（目的地/日期/人数/出行类动词）的问候、寒暄、聊天、能力询问、随口讨论，即便当前未规划、关键字段会缺失，也一律判为 qa，绝不因缺字段判成 clarification。
...
- 纯确认/告别/拒绝（明确表示同意结束、或谢绝继续规划）→ confirm / end_session / reject，不提取 patch。
- 仅当原话确实为"开启/补充一次旅行规划"（含有明确出行目的地、日期、人数等一项或多项规划信息）而字段仍缺时，才判 clarification。
- 实在无法判断 → unknown，不要硬猜。

五、输出约束（严格遵守）
- 只输出一个 JSON object，不要 markdown，不要多余文字。
- 只输出 extracted_request_patch（本轮新增或更新的字段），绝不输出完整 request；本轮没有新信息的字段不要出现。
- 即便意图判为 qa/闲聊，只要原话明确提到目的地/日期等规划字段（如"我想去阳江，有什么好玩的吗"→ destination="阳江"），也必须把这些字段放进 extracted_request_patch，不能因意图是 qa 而清空。
- 不要输出 budget 相关字段。
""".strip()
...
def build_intent_recognition_prompt(intent_input: IntentRecognitionInput) -> str:
    payload = {
        ...
        "session_context": intent_input.session_context.model_dump() if intent_input.session_context else None,
