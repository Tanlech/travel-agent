...
class MemoryStore(Protocol):
    ...
    def count_trip_memories(self, user_id: str | None) -> int: ...
...
class InMemoryStore:
    ...
    def count_trip_memories(self, user_id: str | None) -> int:
        ...
        if not user_id:
            return 0
        return len(self._trip_memory.get(user_id, []))

    def delete_user_data(self, user_id: str | None) -> None:
        """清空该用户的偏好与行程记忆"""
        if not user_id:
            return
        self._user_memory.pop(user_id, None)
        self._trip_memory.pop(user_id, None)
...
class RedisMemoryStore:
    ...
    def count_trip_memories(self, user_id: str | None) -> int:
        ...
        if not user_id:
            return 0
        return int(self.redis.llen(_trip_key(user_id)) or 0)

    def delete_user_data(self, user_id: str | None) -> None:
        """清空该用户的偏好与行程记忆（删除用户时调用）"""
        if not user_id:
            return
        self.redis.delete(_user_key(user_id), _trip_key(user_id))
