...
from app.agent.domain.common.planning import REQUIRED_PATCH_FIELDS, compute_missing_fields
from app.agent.domain.session.schema import SessionMergeResult, SessionRequestState
...
def merge_request_state(
    ...
    for field, value in request_patch.items():
        ...
        if field in LIST_FIELDS:
            ...
            continue
        # 标量字段：直接覆盖（None/空串已在上面跳过）
        if next_dump.get(field) != value:
            next_dump[field] = value
            changed_fields.append(field)
