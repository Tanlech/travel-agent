"""attraction 知识库唯一入库入口 + 运行时沉淀

把某城市的景点清单标准化为知识条目并写入 Qdrant（id = 城市:景点名，幂等覆盖）
...
集中到一处可保证导入与沉淀始终写入同一份格式，避免两套逻辑漂移
...
from app.knowledge.ingest.common import ATTRACTION_COLLECTION
